...
/**
 * MOCKUP — a more detailed skyline, for comparison against the live one.
 *
 * Not wired into the hero. `skyline.tsx` is still what ships.
 *
 * Three things this changes:
 *
 * 1. **Real width variation.** The current version picks widths from one narrow
 *    band, so every tower reads as the same rectangle at a slightly different
 *    height. Width here is a property of the building *type* — spires are
 *    narrow, deco blocks are wide — which spreads them from 18 to 130 units.
 *
 * 2. **Twelve silhouettes instead of one.** Setbacks, tapers, twin peaks,
 *    domes, notched corners, roof water towers, angled crowns.
 *
 * 3. **Construction, in outline.** Two of the types are unfinished buildings:
 *    an open steel lattice above the finished floors, and a tower crane with a
 *    jib and counterweight. The scaffolding is part of the silhouette rather
 *    than a texture on the face, so it still reads at this scale — a facade
 *    pattern would disappear into the window grid.
 */
...
function shapeFor(
  ...
  switch (type) {
    ...
    case "crane": {
      ...
      const mastTop = BASELINE - h - (30 + random() * 26);
      details.push({
        x: mastX - 1,
        y: mastTop,
        w: 2,
...
function stars(random: () => number): Light[] {
  return Array.from({ length: 70 }, () => ({
    x: random() * VIEW_W,
    y: random() * (VIEW_H * 0.32),
    size: 2,
    opacity: 0.14 + random() * 0.3,
  }));
}

export function SkylineV2({ seed = 20260913 }: { seed?: number }) {
  ...
  while (x < VIEW_W + 30) {
    ...
    const h = Math.min(198, Math.max(13, (base + jitter) * damping));
  ...
  return (
    ...
    >
      ...
      >
        ...
        <rect x="0" y={VIEW_H * 0.2} width={VIEW_W} height={VIEW_H * 0.8} fill="url(#cityGlowV2)" />
